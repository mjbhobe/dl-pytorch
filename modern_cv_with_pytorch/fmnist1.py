#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""fmnist1.py - Fashion MNIST dataset classification """
import os, sys
import pathlib
import argparse
...
from utils import get_data, display_sample
...
NUM_EPOCHS, BATCH_SIZE, LEARNING_RATE = 15, 32, 0.001

BASE_DIR = pathlib.Path(__file__).parent.parent
DATA_DIR = os.path.join(BASE_DIR, "data")
MODEL_SAVE_PATH = os.path.join(BASE_DIR, "model_states", "fmnist1.pt")


class Net(nn.Module):
    """ our base model (using ANN)"""

    def __init__(self):
        super(Net, self).__init__()
        self.net = nn.Sequential(
            nn.Flatten(),
            t3.Linear(IMAGE_HEIGHT * IMAGE_WIDTH * NUM_CHANNELS, 1000),
            nn.ReLU(),
            t3.Linear(1000, 1000),
            nn.ReLU(),
            t3.Linear(1000, NUM_CLASSES)
        )
...
def main():
    ...
    if args.show_sample:
        display_sample(test_dataset, class_names, title = "Sample Images from test_dataset")
    ...
    if args.do_training:
        ...
        hist = trainer.fit(
            ...
            lr_scheduler = lr_scheduler, l1_reg = args.l1_reg, l2_reg = args.l2_reg
        )
