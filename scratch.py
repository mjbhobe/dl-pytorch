...
def generate_data() -> NumpyArrayTuple:
    r = np.arange(500)
    x = np.array([[i + 1, i + 2] for i in r[::2]], dtype=float)
    y = np.array([i[0] + i[1] for i in x], dtype=float).reshape(-1, 1)
    return x, y
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(t3.Linear(2, 8), nn.ReLU(), t3.Linear(8, 1))
...
def main():
    ...
    X_val, X_test, y_val, y_test = train_test_split(
        X_test, y_test, test_size=0.20, random_state=SEED
    )
    print(
    ...
    hist = trainer.fit(
        model, opt, train_dataset=(X_train, y_train), validation_dataset=(X_val, y_val)
    )
