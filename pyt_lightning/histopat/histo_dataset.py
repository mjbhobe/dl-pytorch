...
def get_datasets(download_path, force_download=False, force_recreate=False):
    ...
    if (not downsample_base_path.exists()) or force_recreate:
        ...
        downsample_df = labels_df.groupby("label", group_keys=False).apply(lambda x: x.sample(n=downsample_size))
        # split the subset into train: 15,000, eval: 8,000 and test: 2,000
        # downsample_train_df = downsample_df.sample(n=15000)
        downsample_train_df = downsample_df.groupby("label", group_keys=False).apply(lambda x: x.sample(n=15000))
        assert (
        ...
        downsample_eval_df = downsample_rest_df.groupby("label", group_keys=False).apply(lambda x: x.sample(n=8000))
        assert (
