...
class MetricsLogger(Logger):
    ...
    def log_metrics(self, metrics, step):
        ...
        for metric_name, metric_value in metrics.items():
            if metric_name != "epoch":
                ...
                if (
                    not len(self.history["epoch"])
                    or not self.history["epoch"][-1] == metric_value  # len == 0:
    ...
    def plot_metrics(self, title=None, fig_size=None, col_count=3):
        ...
        metrics_tracked = [
            m[m.find("_") + 1 :] for m in data_cols if m.startswith("train")
        ]
                        ...
                        if index < len(metrics_tracked):
                            ...
                            if row_count == 1:
                                ...
                                if f"val_{metric_name}" in data_cols:
                                    ...
                                    ax_title = (
                                        f"Training & cross-val {metric_name} vs epochs"
                                    )
