...
def main(path, batch_size, device, model_str, trace):
    ...
    # Test
    highest_accuracy = eval(testing_dataloader, model_str, model,
                            device, cross_entropy_loss, 0,
                            False, trace)
...
if __name__ == "__main__":
    ...
    device = torch_directml.device(torch_directml.default_device()) \
        if args.device == 'dml' else torch.device(args.device)
    print(f"{args.path}, {args.batch_size}, {device}, {args.model}")
