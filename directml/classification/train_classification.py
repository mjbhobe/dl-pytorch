...
def main(path, batch_size, epochs, learning_rate,
    ...
    for t in range(epochs):
        ...
        # Train
        train(training_dataloader, model, device, cross_entropy_loss,
              learning_rate, momentum, weight_decay, trace,
              model_str, ci_train)

        if not trace and not ci_train:
            # Test
            highest_accuracy = test_classification.eval(testing_dataloader, model_str,
                                                        model, device, cross_entropy_loss,
                                                        highest_accuracy, save_model, False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--momentum', type=float, default=0.9, metavar='M',
                        help='The percentage of past parameters to store.')
